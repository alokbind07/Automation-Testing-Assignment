...
public class AddProviderModal extends BasePage {
    ...
    // Locators
    private final By workflowDropdown = By.xpath("//*[contains(text(), 'Select Onboarding Workflow')]/following::div[@class='selected-list'][1] | //div[@class='selected-list']");
    // Options in Onboarding Workflow list
    private final By workflowOptions = By.xpath("//div[contains(@class, 'option') or contains(@class, 'menu')]//*[contains(text(), 'Onboarding')] | //*[contains(text(), 'Complete_Test_Onboarding_Workflow')]");
    
    private final By addProviderCard = By.xpath("//div[contains(@class, 'modal')]//*[text()='Add Provider']/parent::div | //div[contains(@class, 'modal')]//*[text()='Add Provider'] | //div[contains(@class, 'modal')]//p[text()='Add Provider']");
    
    // Next Button
    private final By nextButton = By.xpath("//button[contains(., 'Next')] | //span[contains(text(), 'Next')] | //button[contains(text(), 'Next')]");

    // Step 2: Privileges Dropdown
    private final By privilegesDropdown = By.xpath("//*[contains(text(), 'Privileges') or contains(text(), 'privilege')]/following::div[contains(@class, 'selected-list')][1]");
    // Generic options list for dropdown
    private final By dropdownOptionsList = By.xpath("//li[not(contains(@class, 'breadcrumb'))] | //div[contains(@class, 'menu-list')]//div[contains(@class, 'option')]");

    // Form inputs
    private final By npiInput = By.xpath("//input[@name='npi' or contains(@placeholder, 'NPI') or @id='npi' or contains(@name, 'npi')]");
    private final By firstNameInput = By.xpath("//input[@name='firstName' or contains(@placeholder, 'First Name') or @id='firstName' or contains(@name, 'first')]");
    private final By lastNameInput = By.xpath("//input[@name='lastName' or contains(@placeholder, 'Last Name') or @id='lastName' or contains(@name, 'last')]");

    // Add button to finish the onboarding creation
    private final By addButton = By.xpath("//div[contains(@class, 'modal') and contains(@class, 'show')]//button[text()='ADD' or contains(text(), 'ADD') or contains(., 'ADD') or contains(text(), 'Add')]");
    
    // Error element for NPI validation
    private final By npiError = By.xpath("//*[contains(text(), 'NPI') and (contains(text(), 'invalid') or contains(text(), 'digit') or contains(text(), '10') or contains(text(), 'valid'))] | //*[contains(text(), 'valid NPI')] | //div[contains(@class, 'error') or contains(@class, 'invalid-feedback')]");
    ...
    public void selectPrivilegeByName(String privilegeName) {
        ...
        By specificOptionLocator = By.xpath("//li[contains(., '" + privilegeName + "') or contains(text(), '" + privilegeName + "')] | //*[contains(@class, 'option') and (contains(., '" + privilegeName + "') or contains(text(), '" + privilegeName + "'))]");
        ...
        if (!opened) {
            logger.warn("Privileges options were not visible after retries. Doing one final click and option search...");
        ...
        // Toggle the privileges dropdown trigger again to close the overlay options list
        try {
            ...
            logger.warn("Could not click privileges dropdown trigger to close overlay, or overlay did not hide: {}", e.getMessage());
        }
    }
    ...
    private void selectFirstAvailablePrivilege() {
        ...
        for (int retry = 0; retry < 3; retry++) {
            try {
                List<WebElement> options = fullWait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(dropdownOptionsList));
    ...
    public void fillDetails(String npi, String firstName, String lastName) {
        ...
        // Clear name inputs first so previous attempt values are not falsely detected as NPPES auto-filled
        try {
        ...
        if (!autoFilled) {
            ...
            // Retrieve auto-filled names and send/dispatch events to sync React/Angular state
